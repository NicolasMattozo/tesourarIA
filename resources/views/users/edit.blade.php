<x-layouts.app title="Novo Usuario">
            ...
            <x-content-wrapper>
                ...
                <x-card>
                    ...
                    <div class="p-6">
                        ...
                        <x-form method="POST" action="{{ route('users.update', $user) }}" name="Usuario">
                            @method('PUT')
                            <x-input.input label="Nome do Usuario" name="name" type="text"
                                placeholder="Digite o nome do usuario" value="{{ old('name', $user->name) }}"
                                required />
                            ...
                            <div>
                                ...
                                <select name="church_section_id"
                                    ...
                                    @foreach ($churchSections as $section)
                                        <option
                                            {{ old('church_section_id', $user->church_section_id) == $section->id ? 'selected' : '' }}
                                            value="{{ $section->id }}">{{ $section->name }}</option>
                                    @endforeach
                                </select>
                            </div>

                            <div x-data="rolesSelect({{ $userRoleIds->toJson() }})" class="space-y-3">

                                <label class="block text-sm font-semibold text-gray-700">
                                    Papéis do Usuário
                                </label>

                                <template x-for="(roleId, index) in roles" :key="index">
    ...
    <script>
        function rolesSelect(preloadedRoles = []) {
            return {
                roles: preloadedRoles.length ? preloadedRoles : [''],
